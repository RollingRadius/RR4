...
class OrganizationService:
    ...
    def get_pending_users(
        ...
        self._check_organization_access(
            current_user_id,
            organization_id,
            required_roles=['logistic_partner', 'load_owner', 'admin']
        )
    ...
    def approve_user(
        ...
        if role_key in ('logistic_partner', 'load_owner'):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    def update_user_role(
        ...
        if user_org.role.role_key in ('logistic_partner', 'load_owner') and \
                current_user_org.role.role_key not in ('logistic_partner', 'load_owner'):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if new_role_key in ('logistic_partner', 'load_owner') and \
                current_user_org.role.role_key not in ('logistic_partner', 'load_owner'):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
