...
def verify_owner(current_user: User, db: Session) -> UserOrganization:
    ...
    owner_org = db.query(UserOrganization).join(
        ...
        Role.role_key.in_(['logistic_partner', 'load_owner']),
...
def update_employee_role(
    ...
    **Parameters:**
    ...
    if emp_org.role and emp_org.role.role_key in ('logistic_partner', 'load_owner'):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    if new_role.role_key in ('logistic_partner', 'load_owner'):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
